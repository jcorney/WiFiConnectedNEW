package com.boomgaarden_corney.android.wifi;
...
public class WiFiConnectedMainActivity extends Activity {

	private final static String DEBUG_TAG = "DEBUG_WIFI";
	private final static String SERVER_URL = "http://54.86.68.241/wificonnected/test.php";
	...
	private static String wifiLinkSpeed;
	private static String wifiRSSI;
	private static String wifiNetworkID;
	private static String wifiFrequency;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_wi_fi_connected_main);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.wi_fi_connected_main, menu);
		return true;
	}
	...
	public static class WiFiBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (counter <= 10) {
				...
				WifiInfo wifiConnectionInfo = mWiFiManager.getConnectionInfo();
				wifiBSSID = wifiConnectionInfo.getBSSID();
				wifiSSID = wifiConnectionInfo.getSSID();
				wifiIPAddress = String.valueOf(wifiConnectionInfo
						.getIpAddress());
				wifiLinkSpeed = String.valueOf(wifiConnectionInfo
						.getLinkSpeed());
				wifiRSSI = String.valueOf(WifiManager.calculateSignalLevel(
						wifiConnectionInfo.getRssi(), 100));
				wifiNetworkID = String.valueOf(wifiConnectionInfo
						.getNetworkId());
				wifiFrequency = String.valueOf(wifiConnectionInfo
						.getFrequency());
				
				paramsWiFi.add(new BasicNameValuePair(
						"-----CONNECTED NETWORK INFORMATION-----", " "));
				paramsWiFi.add(new BasicNameValuePair("Count: ",
						String.valueOf(counter)));
				paramsWiFi.add(new BasicNameValuePair("SSID: ", wifiSSID));
				paramsWiFi.add(new BasicNameValuePair("BSSID: ", wifiSSID));
				paramsWiFi.add(new BasicNameValuePair("IP Address: ",
						wifiIPAddress));
				paramsWiFi.add(new BasicNameValuePair("Link Speed: ",
						wifiLinkSpeed + " Mbps"));
				paramsWiFi
						.add(new BasicNameValuePair(
								"Received Signal Strength Indicator (RSSI): ",
								wifiRSSI));
				paramsWiFi.add(new BasicNameValuePair("Network ID: ",
						wifiNetworkID));
				paramsWiFi.add(new BasicNameValuePair("Frequency: ",
						wifiFrequency + " MHz"));
		...
		private void showWiFiData() {
			...
			results.append("-----CONNECTED NETWORK INFORMATION-----\n");
			results.append("Count: " + counter + "\n");
			results.append("SSID: " + wifiSSID + "\n");
			results.append("BSSID: " + wifiBSSID + "\n");
			results.append("IP Address: " + wifiIPAddress + "\n");
			results.append("Link Speed: " + wifiLinkSpeed + " Mbps\n");
			results.append("Received Signal Strength Indicator (RSSI): "
					+ wifiRSSI + "\n");
			results.append("Network ID: " + wifiNetworkID + "\n");
			results.append("Frequency: " + wifiFrequency + "MHz\n");
